...
train_filenames = [name['image'].split('/')[-1].replace('imagesTr', 'training') for name in train_filenames]
...
for i, filename in enumerate(all_filenames):
    ...
    label = nib.load(f'{basedir}/{patient_id}/{filename.replace(".nii.gz", "_gt.nii.gz")}').get_data()

    n_slices = img.shape[-1]
    
    img = np.array([cv2.resize(img[:, :, i], (512, 512), interpolation=cv2.INTER_LANCZOS4) for i in range(n_slices)]).transpose(1, 2, 0)
    label = np.array([cv2.resize(label[:, :, i], (512, 512), interpolation=cv2.INTER_NEAREST) for i in range(n_slices)]).transpose(1, 2, 0)

    print(splitdir, filename, img.shape, label.shape, (img.mean(), img.std(), img.min(), img.max()), (label.mean(), label.std(), label.min(), label.max()))
   
    for slice_idx in range(n_slices):
        slice_idxs = [max(slice_idx-2, 0), max(slice_idx-1, 0), slice_idx, min(slice_idx+1, n_slices-1), min(slice_idx+2, n_slices-1)]
        img_slices = img[:, :, slice_idxs]
        label_slices = label[:, :, slice_idxs]

        img_new = nib.Nifti1Image(img_slices, np.eye(4))
        nib.save(img_new, ensure_dir(f'{outputdir}/images/{splitdir}/{filename_noext}_{slice_idx:03d}.nii.gz'))
        
        label_new = nib.Nifti1Image(label_slices, np.eye(4))
        nib.save(label_new, ensure_dir(f'{outputdir}/annotations/{splitdir}/{filename_noext}_{slice_idx:03d}.nii.gz'))

        json_metadata[splitdir].append({'image': f'images/{splitdir}/{filename.replace(".nii.gz", "")}_{slice_idx:03d}.nii.gz', 'label': f'annotations/{splitdir}/{filename_noext}_{slice_idx:03d}.nii.gz'})
...
for i, filename in enumerate(all_filenames):
    ...
    for slice_idx in range(n_slices):
        ...
        json_metadata[splitdir].append(f'images/{splitdir}/{filename.replace(".nii.gz", "")}_{slice_idx:03d}.nii.gz')
