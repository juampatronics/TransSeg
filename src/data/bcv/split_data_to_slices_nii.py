...
for i, idx in enumerate(file_idxs):
    ...
    print(splitdir, i, idx, img.shape, label.shape, (img.mean(), img.std(), img.min(), img.max()), (label.mean(), label.std(), label.min(), label.max()))
    for slice_idx in range(n_slices):
        ...
        nib.save(img_new, ensure_dir(f'{outputdir}/images/{splitdir}/img{idx:04d}_{slice_idx:03d}.nii.gz'))
        
        label_new = nib.Nifti1Image(label_slices, np.eye(4))
        nib.save(label_new, ensure_dir(f'{outputdir}/annotations/{splitdir}/label{idx:04d}_{slice_idx:03d}.nii.gz'))
        json_metadata[splitdir].append({'image': f'images/{splitdir}/img{idx:04d}_{slice_idx:03d}.nii.gz', 'label': f'annotations/{splitdir}/label{idx:04d}_{slice_idx:03d}.nii.gz'})
...
for i, idx in enumerate(file_idxs):
    ...
    for slice_idx in range(n_slices):
        ...
        json_metadata[splitdir].append(f'images/{splitdir}/img{idx:04d}_{slice_idx:03d}.nii.gz')
